...
class bilstm(nn.Module):
    def __init__(self,input_dim,hidden_dim=16,output_dim,num_layers,dropout=0.5):
...
for lag in [5,10,20,30]:
	for norm_volume in ["v1","v2"]:
		...
		# the function we have wrapped for the pipeline
		X_tr, y_tr, X_va, y_va, X_te, y_te,norm_params = load_data_v5(None, horizon, ["LME_Co_Spot"], lag, 
                                                                "NEXT", split_dates, 
		...
		for epoch in range(EPOCH):
    		while batch_len<len(train_X)-1:   # gives batch data
