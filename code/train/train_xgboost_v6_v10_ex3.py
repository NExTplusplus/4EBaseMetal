    ...
    if args.action == 'train':
        ...
        for f in fname_columns:
            ...
            for split_date in split_dates:
                ...
                # final_X_tr = np.insert(final_X_tr,0,np.array(range(np.shape(final_X_tr)[0]*np.shape(final_X_tr)[1])).reshape([np.shape(final_X_tr)[0],np.shape(final_X_tr)[1]]),axis = 2)
                # final_y_tr = np.insert(final_y_tr,0,np.array(range(np.shape(final_y_tr)[0]*np.shape(final_y_tr)[1])).reshape([np.shape(final_y_tr)[0],np.shape(final_y_tr)[1]]),axis = 2)
                np.random.seed(10)
                np.random.shuffle(final_X_tr)
                np.random.seed(10)
                np.random.shuffle(final_y_tr)
