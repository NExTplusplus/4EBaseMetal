...
class Trainer:
    ...
    def train_minibatch(self, num_epochs, batch_size, interval):
        ...
        val_size = len(new_val_X)
        test_size = len(new_test_X)
        print(len(new_val_X))
...
#            train_loss_list.append(float(train_loss))
            ...
            if if_eval_train:
                while i < val_size:
                    ...
                    if batch_end > val_size:
                        batch_end = val_size
                    val_X_repredict=np.array(new_val_X[i:batch_end])
                    val_Y_repredict=np.array(new_val_y[i:batch_end])
                    val_X = torch.from_numpy(val_X_repredict).float()
                    val_Y = torch.from_numpy(val_Y_repredict).float()
                    var_x_val_id = torch.LongTensor(np.array(new_val_embedding[i:batch_end]))
                    #print(val_X.shape)
            ...
            while i < test_size:
                ...
                if batch_end > test_size:
                    batch_end = test_size
                test_X_repredict=np.array(new_test_X[i:batch_end])
                test_Y_repredict=np.array(new_test_y[i:batch_end])
                test_X = torch.from_numpy(test_X_repredict).float()
                test_Y = torch.from_numpy(test_Y_repredict).float()
                var_x_test_id = torch.LongTensor(np.array(new_test_embedding[i:batch_end]))
