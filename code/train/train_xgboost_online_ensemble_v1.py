...
if __name__ == '__main__':
    ...
    if args.action == 'train':
        ...
        for f in fname_columns:
            ...
            importance_list = []
            version_v5 = generate_version_params('v5')
            version_v10 = generate_version_params('v10')
            version_params_v5=generate_version_params(version_v5)
            version_params_v10=generate_version_params(version_v10)
            for split_date in split_dates:
                for horizon in [1]:
                    for lag in [5,10,20,30]:
                        ...
                        X_tr, y_tr, X_va, y_va, X_te, y_te, norm_params,column_list = load_data(ts,LME_dates,horizon,args.ground_truth,lag,split_date,norm_params,tech_params,version_params_v5)
                        column_lag_list = []
                        column_name = []
                        for i in range(lag):
                        ...
                        if lag == 5 and horizon==1:
                            learning_rate = 0.7
                            max_depth = 4
                            gamma = 0.9
                            min_child_weight = 3
                            subsample = 0.6
                        elif lag == 10 and horizon==1:
                            learning_rate=0.6
                            max_depth = 5
                            gamma = 0.6
                            min_child_weight=5
                            subsample=0.7
                        elif lag==20 and horizon==1:
                            learning_rate = 0.8
                            max_depth = 3
                            gamma = 0.8
                            min_child_weight=6
                            subsample=0.6
                        elif lag==30 and horizon==1:
                            learning_rate = 0.7
                            max_depth = 6
                            gamma = 0.9
                            min_child_weight=5
                        ...
                        if args.voting=='all':
                            result_v5 = np.concatenate((folder_1,folder_2,folder_3,folder_4,folder_5,folder_6,folder_7,folder_8,folder_9,folder_10),axis=1)
                        #start to construct the v10 result
                        norm_params = {'vol_norm':norm_volume,'ex_spread_norm':norm_ex,'spot_spread_norm':norm_3m_spread,
                                        'len_ma':len_ma,'len_update':len_update,'both':3,'strength':0.01,'xgboost':False}
                        ts = copy(time_series.loc[split_date[0]:split_date[2]])
                        X_tr, y_tr, X_va, y_va, X_te, y_te, norm_params,column_list = load_data(ts,LME_dates,horizon,args.ground_truth,lag,split_date,norm_params,tech_params,version_params_v10)
                        column_lag_list = []
                        column_name = []
                        for i in range(lag):
                            for item in column_list[0]:
                        ...
                        #calculate the all folder voting
                        if args.voting=='all':
                            result_v10 = np.concatenate((folder_1,folder_2,folder_3,folder_4,folder_5,folder_6,folder_7,folder_8,folder_9,folder_10),axis=1)
                        final_list = []
                        for j in range(len(result_v10)):
                            probablity = 0
                            for item in result_v10[j]:
                                probablity+=item
                            for new_item in result_v5[j]:
                                probablity+=new_item
                            result = float(probablity)/float(len(result_v5[j]))
                            if result>0.5:
                                final_list.append(1)
                            else:
                                final_list.append(0)
                        print("the v1 ensemble precision is {}".format(metrics.accuracy_score(y_va, final_list)))
