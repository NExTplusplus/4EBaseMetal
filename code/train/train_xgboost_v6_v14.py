    ...
    if args.action == 'train':
        ...
        for f in fname_columns:
            ...
            for ground_truth in ['LME_Co_Spot','LME_Al_Spot','LME_Ni_Spot','LME_Ti_Spot','LME_Zi_Spot','LME_Le_Spot']:
                target = ground_truth.split('_')[-2]
                futures = "LME_"+target+"_Close"
                time_series["LME_"+target+"_Spread"] = time_series[ground_truth]-time_series[futures]
            ...
            for split_date in split_dates:
                ...
                for ground_truth in ['LME_Co_Spot','LME_Al_Spot','LME_Ni_Spot','LME_Ti_Spot','LME_Zi_Spot','LME_Le_Spot']:
                    ...
                    X_tr = X_tr.reshape(len(X_tr),nth*len(column_list[0]))
                    X_tr = np.append(X_tr,[metal_id]*len(X_tr),axis = 1)
                    y_tr = np.concatenate(y_tr)
                    X_va = np.concatenate(X_va)
                    X_va = X_va.reshape(len(X_va),nth*len(column_list[0]))
