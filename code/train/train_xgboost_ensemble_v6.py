    ...
    if args.action == 'train':
        ...
        for f in fname_columns:
            ...
            for split_date in split_dates:
                ...
                #result_v5 = np.loadtxt(args.ground_truth[0]+"_horizon_"+str(horizon)+"_"+split_date[1]+"_"+"v5"+".txt")
                ...
                if len(result_v7_error)==0:
                    for i in range(len(result_v7)):
                        count_1=0
                        count_0=0
                    ...
                    for i in range(window_size,len(y_va)):
                        ...
                        #weight_xgb_v5 = float(1/error_xgb_v5)/(1/error_xgb_v7+1/error_xgb_v10+1/error_lr)
                        weight_xgb_v7 = result_v7_previous_weight[length]*weight+(1-weight)*float(1/error_xgb_v7)/(1/error_xgb_v7+1/error_xgb_v10+1/error_lr)
                        weight_xgb_v10 = result_v10_previous_weight[length]*weight+(1-weight)*float(1/error_xgb_v10)/(1/error_xgb_v7+1/error_xgb_v10+1/error_lr)
                        weight_lr = result_lr_previous_weight[length]*weight+(1-weight)*float(1/error_lr)/(1/error_xgb_v7+1/error_xgb_v10+1/error_lr)
                        ...
                        result+=weight_xgb_v10*final_list_v10[j]
                        if result_lr[i]>0.5:
                            result+=weight_lr
                        if result>0.5:
                            final_list.append(1)
                        ...
                        length+=1
                    print("the weight 2 ensebmle for V7 V10 LR voting precision is {}".format(metrics.accuracy_score(true_result, final_list)))
                    ...
                    for i in range(0,len(y_va)):
                        ...
                        for i in range(len(final_list_v7)):
                            if final_list_v7[i]+final_list_v10[i]+final_lr[i]>=2:
                                voting_final.append(1)
                            else:
                                voting_final.append(0)
                        length+=1
                    print("the weight 2 ensebmle for V7 V10 LR voting precision is {}".format(metrics.accuracy_score(true_result, final_list)))
                    print("the voting result is {}".format(metrics.accuracy_score(true_result, voting_final)))
                    print("the horizon is {}".format(horizon))
                    print("the lag is {}".format(lag))
                    print("the train date is {}".format(split_date[0]))
                    print("the test date is {}".format(split_date[1]))