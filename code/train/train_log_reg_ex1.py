...
from data.load_data_v5_ex1 import load_data_v5_ex1
                    ...
                    for C in [0.00001,0.0001,0.001,0.01]:
                        for norm_volume in ["v1","v2"]:
                            for norm_ex in ["v1"]:
                                ...
                                # load data
                                X_tr, y_tr, X_va, y_va, X_te, y_te,norm_params = load_data_v5_ex1(f, horizon, args.ground_truth, lag, 
                                ...
                                if len(args.ground_truth) == 1:
                                    pure_LogReg.save(os.path.join(sys.path[0],args.model_save_path,"exp",str(horizon)+"d",args.ground_truth[0][4:6],"logistic_regression_ex1","v"+str(args.version),"n"+str(n)+".joblib"))
                                else:
                                    pure_LogReg.save(os.path.join(sys.path[0],args.model_save_path,"exp",str(horizon)+"d","All","logistic_regression_ex1","v"+str(args.version),"n"+str(n)+".joblib"))
                                out.write(str(C)+",")
                                out.write(str(lag)+",")
