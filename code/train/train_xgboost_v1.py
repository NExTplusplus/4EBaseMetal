    ...
    if args.action == 'train':
        ...
        for f in fname_columns:
            ...
            for split_date in split_dates:
                ...
                #print(train_X.index)
                #print(train_Y.index)
                # we use the ten folders for the reason that our validation data is just ten percent of the train data
                ...
                for fold_n, (train_index, valid_index) in enumerate(folds.split(train_X)):
                    ...
                    np.savetxt(str(lag)+"_"+str(split_date[1])+"_"+args.ground_truth[0]+"_"+str(fold_n)+".txt", prediction_each_folder)
                    #print("the mean of prediction in this folder is {}".format(np.mean(y_pred)))
                    #print("the std of prediction in this folder is {}".format(np.std(y_pred)))
                    #for i in range(len(prediction_each_folder)):
                    #    if prediction_each_folder[i]>=0.5:
                    #        prediction_each_folder[i]=1
                    #    elif prediction_each_folder[i]<0.5:
                    #        prediction_each_folder[i]=0
                    #print("in this folder the prediction accuracy is {}".format(metrics.accuracy_score(y_va, prediction_each_folder)))
                    #oof[valid_index] = y_pred_valid.reshape(-1, 1)
                    #scores.append(metrics.accuracy_score(y_valid, y_pred_valid))
                    #prediction += y_pred
                #prediction /= n_splits
                ...
                #        prediction[i]=0
                #print('The test score is {}'.format(metrics.accuracy_score(y_va, prediction)))
