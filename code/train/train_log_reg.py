    ...
    with open(args.output+".csv","w") as out:
        ...
        if args.action == 'train':
            ...
            for f in fname_columns:
                    ...
                    for C in (0.01,0.1,1,10,100):
                        for norm_ex in ["v2"]:
                            for norm_volume in ["v1","v2","v4"]:
                                ...
                                # plt.close()
                                pure_LogReg.save("4EBaseMetal/exp/log_reg/"+str(args.ground_truth)+"_h"+str(args.steps)+"_n"+str(n)+"_v"+str(args.version)+".joblib")
                                if acc > max_acc:
                                    best_C = C
                                    best_lag = lag
                                    model = pure_LogReg
                ...
                X_tr, y_tr, X_va, y_va, X_te, y_te,norm_params= load_pure_log_reg(
                    f, 'log_1d_return', split_dates,gt_column = args.ground_truth, T = best_lag,S = horizon,
