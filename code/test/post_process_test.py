# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 13:37:52 2019
...
if __name__ == '__main__':
    ...
    n = 0
    target = args.ground_truth.split('_')[-2]
    futures = "LME_"+target+"_Close"
#    print(futures)
#    for col in time_series.columns:
#        if target in col:
#            print(col)
    time_series["LME_"+target+"_Spread"] = time_series[args.ground_truth] - time_series[futures]
    ...
    strategy_params = {'sar':{'initial':[],'maximum':[]},'rsi':{'window':[],'upper':[],'lower':[]},'strat1':{'short window':[],"med window":[]},
                              'strat2':{'window':[]},'strat3_high':{'window':[]},'strat3_close':{'window':[]},'strat6':{'window':[],'limiting_factor':[]},
                              'strat7':{'window':[],'limiting_factor':[]},'strat8':{'window':[],'limiting_factor':[]},'strat9':{'SlowLength':[],'FastLength':[],'MACDLength':[]}}
    activation_params = {'sar':False,'rsi':False,'strat1':False,'strat2':False,'strat3_high':False,'strat3_close':False, 'strat6':False, 'strat7':False, 'strat8':True,'strat9': False}
    strat_results = {'sar':{'initial':[],'maximum':[]},'rsi':{'window':[],'upper':[],'lower':[]},'strat1':{'short window':[],"med window":[]},'strat2':{'window':[]},'strat3_high':{'window':[]}, 'strat3_close':{'window':[]},'strat6':{'window':[],'limiting_factor':[]},
                            'strat7':{'window':[],'limiting_factor':[]},'strat8':{'window':[],'limiting_factor':[]},'strat9':{'SlowLength':[],'FastLength':[],'MACDLength':[]}}
...
#    rsi = post_process(copy(ts), split_dates, "rsi", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#
#    print("strat1")
...
#    strat1 = post_process(copy(ts), split_dates, "strat1", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#    
#    print("strat2")
...
#    strat2 = post_process(copy(ts), split_dates, "strat2", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#
#    print("strat3_high")
...
#    comb = [[com] for com in comb]
#    strat3_high = post_process(copy(ts), split_dates, "strat3_high", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#
#    print("strat3_close")
...
#    comb = list(range(5,51,2))
#    comb = [[com] for com in comb]
#    strat3_close = post_process(copy(ts), split_dates, "strat3_close", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#
#    print("strat6")
...
#    strat6 = post_process(copy(ts), split_dates, "strat6", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
...
#    window = range(10,51,2)
#    comb = product(window,limiting_factor)
#    strat7 = parallel_process(copy(ts), split_dates, "strat7", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
#    
#    print('strat9')
#    activation_params['strat7'] = False
#    activation_params['strat9'] = True
#    comb = list(permutations(range(10,51,5),3))
#    strat9 = post_process(copy(ts), split_dates, "strat9", strat_results, ground_truth, strategy_params,activation_params,comb,args.min)
    print("strat8")
    window = range(6,80,2)
    limiting_factor = np.arange(1.8,2.45,0.1)
    comb = product(window,limiting_factor)
    strat8 = parallel_process(copy(ts), split_dates, "strat8", strat_results, ground_truth, strategy_params,activation_params,args.cov_increase,comb,args.min)
...
#          }
    ans = {'index':[],'strat8_window':[],'strat8_limiting_factor':[],'strat8_train_acc':[],'strat8_train_cov':[],'strat8_acc':[],'strat8_cov':[]}
...
#                ans['strat7_cov'].append(None) 
        for i in range(mx):
             ...
             if i < len(strat_results['strat8']['window']):
                strat_dc = create_dc_from_comb("strat8",strategy_params,[strat_results['strat8']['window'][i],strat_results['strat8']['limiting_factor'][i]])
                train_results = output(ts, split_dates,ground_truth,strategy_params,activation_params,strat_dc)
                results = output(ts, test_split_date,ground_truth,strategy_params,activation_params,strat_dc, check = False)
                ans['strat8_window'].append(strat_results['strat8']['window'][i])
                ans['strat8_limiting_factor'].append(strat_results['strat8']['limiting_factor'][i])
                ans['strat8_train_acc'].append(train_results[3])
                ans['strat8_train_cov'].append(train_results[4])
                ans['strat8_acc'].append(results[3])
                ans['strat8_cov'].append(results[4]) 
             else:
                ans['strat8_window'].append(None)
                ans['strat8_limiting_factor'].append(None)
                ans['strat8_train_acc'].append(None)
                ans['strat8_train_cov'].append(None)
                ans['strat8_acc'].append(None)
                ans['strat8_cov'].append(None) 
...
#                ans['strat9_cov'].append(None)
    pd.DataFrame(ans).to_csv(args.output)
