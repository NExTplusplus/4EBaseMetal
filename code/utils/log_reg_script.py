                ...
                for gt in ground_truth_list:
                    for h in args.step_list:
                        ...
                        for lag in args.lag_list:
                            temp = pd.read_csv('_'.join(["log_reg",gt,version,lag,h])+".csv")
                            f = pd.concat([pd.DataFrame({"lag":[lag]*len(temp)}),f], axis = 1)
                            total = pd.concat([total,f],axis = 0)
                        total.reset_index(inplace =True,drop = True)
                        temp = {"average":[0]*len(temp)*len(args.lag_list), "length":[0]*len(temp)*len(args.lag_list)}
                        for col in total.columns.values.tolist():
                            if "_length" in col and (sum(total[col]) < 1000*len(temp)*len(args.lag_list)):
                                split_date = col[:-7]
                                temp['average'] = [sum(x) for x in zip(temp['average'],list(total[split_date+"_acc"]*total[col]))]
                                temp['length'] = [sum(x) for x in zip(temp['length'],list(total[col]))]
                        temp = pd.DataFrame({"true_average":np.true_divide(temp['average'],temp['length'])})
        ...
        for version in args.version_list:
            ...
            for h in args.step_list:
                for gt in ground_truth_columns:
                    f = pd.read_csv("log_reg_"+version+"_final/"+'_'.join(["log_reg_online",version,gt,h])+".csv")
                    total = pd.concat([total,f],axis = 0)
                total.reset_index(inplace =True,drop = True)
        total.to_csv("check_normal_final.csv")
