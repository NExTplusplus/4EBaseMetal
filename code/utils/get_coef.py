...
print(os.path.abspath(os.path.join(sys.path[0],"exp","log_reg")))
directory = os.listdir(os.path.abspath(os.path.join(sys.path[0],"exp","log_reg")))


def get_coef(gt,h,n,v,results):
  coef = None
  if gt+"_h"+str(h)+"_n"+str(n)+"_v"+str(v)+".joblib" in directory:
    model = joblib.load(os.path.abspath(os.path.join(sys.path[0],"exp","log_reg",gt+"_h"+str(h)+"_n"+str(n)+"_v"+str(v)+".joblib")))
    coef = model.coef_
  if coef is not None:
    col =""
    with open(results) as f:
      ...
      if v in [2,4,6]:
        col = 'self,'+col
      with open(os.path.abspath(os.path.join(results,"..",)),"w") as out:
        out.write(col+"\n")
