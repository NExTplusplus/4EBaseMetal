...
def three_classifier_labelling(arguments,split_dates):
    # contruct the three classifier labels
    time_series = arguments['time_series']
    horizon = arguments['horizon']
    ground_truth_columns = arguments['ground_truth_columns']
    ...
    for ground_truth in ground_truth_columns:
        ...
        #print(split_dates)
        mean = np.mean(labels[:train_end])
        std = np.std(labels[:train_end])
        #calculate the distribution of the train data
        threshold_1 = sct.norm.ppf(q=0.309,loc=mean,scale=std)
        threshold_2 = sct.norm.ppf(q=0.691,loc=mean,scale=std)
        train_labels = labels[:train_end]
        val_labels = labels[train_end:val_end]
        #calculate the len of the distribution
        train_labels[(train_labels <= threshold_1)&(train_labels.index <= train_end)] = -1
        train_labels[(train_labels <= threshold_2)&(train_labels > threshold_1)&(train_labels.index <= train_end)] = 0
        train_labels[(train_labels > threshold_2)&(train_labels.index <= train_end)] = 1
        val_labels[(val_labels <= threshold_1)&(val_labels.index <= val_end)] = -1
        val_labels[(val_labels <= threshold_2)&(val_labels > threshold_1)&(val_labels.index <= val_end)] = 0
